...
try:
    ...
    try:
        ...
        try:
            ...
            exit(0)
import json
API_ID = '21435901' #api id
API_HASH = 'ba9412cea24b23fcfb42c98ae0bf55f4' #api hash
bot_token = "" # 6605841064:AAFgPu-ikMzHdF3h5BiWOkaxbJ5nLrHwixI
running_processes = {}
try:
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if (update.message.chat.type == "private"):
        if (str(update.message.chat.id) == str(info["sudo"])):
            ...
            await update.message.reply_text("مرحبا بك في سورس التجميع الخاص ببوتات زينب :\n\n- اشترك في قناة تحديثات بوت التجميع : @p_2_8", reply_markup=reply_markup)
        else:
            if not os.path.isdir("echo_ac/"+str(update.message.chat.id)):
                os.makedirs("echo_ac/"+str(update.message.chat.id))
            ...
            reply_markup = InlineKeyboardMarkup(keyboard)
            await update.message.reply_text("مرحبا بك في سورس التجميع الخاص ببوتات زينب :", reply_markup=reply_markup)
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if (query.data == "addecho"):
        ...
        if (str(query.message.chat.id) == str(info["sudo"])):
            ...
            reply_markup = InlineKeyboardMarkup(keyboard)
            await query.edit_message_text("مرحبا بك في سورس التجميع الخاص ببوتات زينب :", reply_markup=reply_markup)
        elif (str(query.message.chat.id) in info["admins"]):
            keyboard = [
                [
                    InlineKeyboardButton(
